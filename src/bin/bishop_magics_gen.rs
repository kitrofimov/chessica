...
fn find_magic(square: usize, mask: u64, shift: u8) -> (u64, Vec<u64>) {
    ...
    let attacks: Vec<u64> = variations.iter()
        .map(|&blockers| bishop_attacks(square, blockers))
        .collect();
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let file = File::create("src/constants/bishop_magics.rs").expect("Failed to create file");
    let mut writer = BufWriter::new(file);

    writeln!(writer, "pub static BISHOP_MAGICS: [u64; 64] = [")?;
    for magic in &magics {
        writeln!(writer, "    0x{:016x},", magic)?;
    }
    writeln!(writer, "];\n")?;

    writeln!(writer, "pub static BISHOP_MAGICS_SHIFT: [u64; 64] = [")?;
    for shift in &shifts {
    ...
    writeln!(writer, "pub static BISHOP_ATTACK_TABLES: [[u64; 1 << {}]; 64] = [", max_index_size)?;
    for table in &tables {
        writeln!(writer, "    [")?;
        let mut i = 0;
        for attack in table {
